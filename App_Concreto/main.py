...
# ---------------------------
# Inicialização do histórico no session_state
# ---------------------------
...
def salvar_historico(tipo: str, resultado: dict):
    ...
    st.session_state["historico_calculos"].append(registro)

def mostrar_historico():
    if st.session_state["historico_calculos"]:
        df = pd.DataFrame([
            {"Elemento": r["tipo"], **r["resultado"]}
            for r in st.session_state["historico_calculos"]
        ])
        st.dataframe(df)
            ...
            if st.button("Calcular Estaca", key="calcular_estaca"):
                try:
                    ...
                    if diametro <= 0 or profundidade <= 0:
                        raise ValueError("Dimensões inválidas. Todas devem ser maiores que zero.")
                    ...
                    salvar_historico("Estaca", materiais)
